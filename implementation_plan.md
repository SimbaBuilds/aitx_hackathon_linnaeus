...
- **M1 — Dev-candle stand-in = Claude Sonnet 5** (`claude-sonnet-5`; revised 2026-07-18, was Opus 4.8), accessed *through* the `CandleClient` seam (Contract B) so it swaps to vLLM-Nemotron with a one-line config change. Dev-only (near-Opus agentic/tool-use quality so the engine is exercise-able reliably, at ~2.5× lower cost than Opus — intro $2/$10 per MTok through 2026-08-31; drop to Haiku 4.5 for high-volume mechanical iteration). **Never used for a measured/scored run** — those are the pinned Nemotron candle only. Needs `ANTHROPIC_API_KEY` (see `camerons_task.md`).
