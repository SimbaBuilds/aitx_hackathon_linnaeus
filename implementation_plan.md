...
- **M1 — Dev-candle stand-in = Claude Opus 4.8**, accessed *through* the `CandleClient` seam (Contract B) so it swaps to vLLM-Nemotron with a one-line config change. Dev-only (real reasoning so the engine is exercise-able before the GPU is up); cost trivial; **never used for a measured/scored run** — those are the pinned Nemotron candle only. Needs `ANTHROPIC_API_KEY` (see `camerons_task.md`).
